...
public class ParametroController {
    ...
    // POST /api/parametros/{id}/metodologias  body: { metodologiaId, matrizId?, tipoAnalisis?, grupoInformeId? }
    ...
    public ResponseEntity<ParametroMetodologiaDE> addMetodologia(
        ...
        Long grupoInformeId  = body.get("grupoInformeId")  != null ? ((Number) body.get("grupoInformeId")).longValue()  : null;
        String tipoAnalisis  = body.get("tipoAnalisis")    != null ? body.get("tipoAnalisis").toString()                : null;
        ParametroMetodologiaDE pm = parametroService.addMetodologia(id, metodologiaId, matrizId, tipoAnalisis, grupoInformeId);
        return ResponseEntity.status(201).body(pm);
    }

    // PATCH /api/parametros/{id}/metodologias/{pmId}  body: { tipoAnalisis?, grupoInformeId? }
    @PatchMapping("/{id}/metodologias/{pmId}")
    public ResponseEntity<ParametroMetodologiaDE> updateMetodologia(
            @PathVariable Long id,
            @PathVariable Long pmId,
            @RequestBody Map<String, Object> body) {
        String tipoAnalisis = body.get("tipoAnalisis") != null ? body.get("tipoAnalisis").toString() : null;
        Long grupoInformeId = body.get("grupoInformeId") != null ? ((Number) body.get("grupoInformeId")).longValue() : null;
        ParametroMetodologiaDE pm = parametroService.updateMetodologia(id, pmId, tipoAnalisis, grupoInformeId);
